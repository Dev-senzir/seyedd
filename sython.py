...
async def _(event):
    ...
    await event.edit(f'''
**❖╎𝚆𝙴𝙻𝙺𝙾𝙼𝙴 𝚃𝙾 𝚂𝙾urce 𝙴𝙻𝙴𝚂𝚈𝙴𝙳 ᴠ₂𐇮
╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌------------
❖╎ᴠᴇʀѕɪᴏɴ𖤓 : ❸.❸
❖╎ᴘɪɴɢ𖤴 : {ms}
❖╎𝙳𝙰𝚃𝙴𖥄 : {m9zpi}
❖╎𝙸𝙳𐇗 : {event.sender_id}
❖╎ѕᴏᴜʀᴄᴇ ᴇʟᴇѕʏᴇᴅ⸎: @Source_elesyed
❖╎ᴅᴇᴠ⛤: @IC_X_K
-------------------------------------------------**
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك الاسياد ابشر @Source_elesyed')
...
async def _(event):
        ...
        await sython.send_message('@zmmbot', 'جاري التجميع بواسطة | YOUSEF TEAM')
        channel_entity = await sython.get_entity(bot_username)
...
async def _(event):
        ...
        await sython.send_message('@A_MAN9300BOT', 'جاري التجميع بواسطة | YOUSEF TEAM')
        channel_entity = await sython.get_entity(bot_usernamee)
...
async def _(event):
      await event.reply("""الاسياد :@Source_elesyedJ"""
)
